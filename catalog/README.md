...
Lightweight CherryPy REST API that stores the system catalog: users, rooms, devices, services, and the ThingSpeak channel pool. Data is persisted in `catalog.json` (Docker volume) and device templates are read from `device_templates.json`.
...
> `POST /devices` returns **405** — devices are only created via `POST /rooms` (the catalog mints them automatically from templates).

### Rooms — `/rooms`
- `GET /rooms` → full list.
- `GET /rooms/{roomID}` → single room.
- `POST /rooms` — **atomic room creation**:
  - Body: `{userID, roomName, times, threshold_parameters}` (do NOT include IDs — the server mints them).
  - Server mints `roomID` (`rm-<hex>`), allocates a ThingSpeak pool slot, instantiates 3 devices from `device_templates.json` (minting `dev-<hex>` for each), writes `roomsList` and `devicesList`, updates the pool.
  - Returns the full room object including `connected_devices` with the minted device IDs.
  - If any step fails → full rollback.
  - Returns 503 if the ThingSpeak pool has no available slots.
- `PATCH /rooms/{roomID}` → partial update (`roomName`, `times`, `threshold_parameters`).
- `DELETE /rooms/{roomID}` — **cascade delete**: removes the 3 linked devices, releases the ThingSpeak slot back to the pool, deletes the room.

### Users — `/users`
- Standard CRUD as above.
- `DELETE /users/{userID}` — **cascade delete**: for each room owned by the user, runs the room cascade (delete devices, release slot), then deletes the user.
...
```json
{
  ...
  "connected_devices": [
    { "deviceID": "dev-<hex>", "role": "HealthMonitorPPG" },
    { "deviceID": "dev-<hex>", "role": "EnvMonitorDHT" },
    { "deviceID": "dev-<hex>", "role": "ActuatorControl" }
  ]
}
```
...
| 503 | ThingSpeak pool exhausted (no available slots) |
